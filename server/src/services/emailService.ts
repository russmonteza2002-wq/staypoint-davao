/**
 * Email Service — Powered by Brevo v3 HTTP API
 * 
 * Uses Brevo's HTTPS REST API (Port 443) to deliver transactional emails 
 * (OTP codes & Manager replies) to any inbox worldwide without cloud firewall blocks.
 */
 ...
 */
async function sendBrevoEmail(params: SendMailParams): Promise<void> {
  const apiKey = process.env.BREVO_API_KEY;
  const senderEmail = process.env.BREVO_SENDER_EMAIL || 'russmonteza2002@gmail.com';

  if (!apiKey) {
    console.error('❌ [Brevo Email] Missing BREVO_API_KEY environment variable.');
    return;
  }
  ...
  if (!response.ok) {
    ...
    console.error('❌ [Brevo Email] API Call Failed:', response.status, errorData);
